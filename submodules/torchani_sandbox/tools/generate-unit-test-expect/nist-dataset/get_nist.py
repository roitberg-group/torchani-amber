...
urltemplate = (
    "https://webbook.nist.gov/cgi/cbook.cgi?Value={}-{}&VType=MW&Formula="
    + formula
    + "&NoIon=on&MatchIso=on"
)  # noqa: E501
min_weight = 0
max_weight = 9999999
...
class NISTSpider(scrapy.Spider):
    ...
    def parse_range(self, response, from_, to):
        ...
        if len(search_result_list) == 400 and to - from_ > 1:
            ...
            yield scrapy.Request(
                url=next_url1, callback=lambda x: self.parse_range(x, from_, mid)
            )
            yield scrapy.Request(
                url=next_url2, callback=lambda x: self.parse_range(x, mid, to)
            )
        elif len(search_result_list) == 400 and to - from_ == 1:
            next_url1 = urltemplate.format(from_, from_)
            next_url2 = urltemplate.format(to, to)
            yield scrapy.Request(
                url=next_url1, callback=lambda x: self.parse_range(x, from_, from_)
            )
            yield scrapy.Request(
                url=next_url2, callback=lambda x: self.parse_range(x, to, to)
            )
        else:
            for i in search_result_list:
                ...
                url3d = (
                    "https://webbook.nist.gov/cgi/cbook.cgi?Str3File=" + nist_id
                )  # noqa: E501
                yield scrapy.Request(
                    url=url3d, callback=lambda x, n=nist_id: self.parse_sdf(x, n)
                )
